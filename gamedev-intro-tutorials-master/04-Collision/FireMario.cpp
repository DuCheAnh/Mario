...
void FireMario::Render(Camera* camera)
{
	...
	if (ani != ANI_FIRE_MARIO_ATTACK)
		animation_set[ani]->Render(camPos.x, camPos.y + crouchdiff, direction);
	else if (ani == ANI_FIRE_MARIO_ATTACK)
	{
		animation_set[ani]->Render(camPos.x, camPos.y + crouchdiff, FIREMARIO_ANI_ATTACKTIME, direction,1,255);
	}
