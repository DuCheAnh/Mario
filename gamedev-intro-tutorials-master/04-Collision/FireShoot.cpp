...
void FireShoot::Update(DWORD dt, std::vector<LPGAMEOBJECT>* coObjects)
{
	...
	else
	{
		...
		for (UINT i = 0; i < coEventsResult.size(); i++)
		{
			...
			else if (e->nx != 0 && nx != 0) isACtive = false;
		}

	}
	Camera* camera = ScenceManager::GetInstance()->getCurrentScence()->getCamera();
	if(this->x - BULLET_BBOX_WIDTH < camera->getCameraPositionX()) isACtive = false;
	if (this->x > camera->getCameraPositionX() + camera->getCameraWidth()) isACtive = false;
